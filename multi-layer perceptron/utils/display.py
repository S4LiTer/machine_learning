...
class Window:
    def __init__(self, width, height, values_per_graph = 100):
        ...
        self.last_y_value = self.height-self.grid_offset
    ...
    def add_point(self, y_value):
        if self.data_count >= self.values_per_graph:
            self.reset()
            

        x_start = self.grid_offset + self.data_count*self.x_step
        self.data_count += 1

        current_y_value = self.height-self.grid_offset-((y_value/10)*self.y_step)


        self.canvas.create_line(x_start, self.last_y_value, x_start+self.x_step, current_y_value, fill=self.line_color, width=2)

        self.last_y_value = current_y_value
